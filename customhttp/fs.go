...
func (d Dir) Open(name string) (File, error) {
	if filepath.Separator != '/' && strings.ContainsRune(name, filepath.Separator) {
		return nil, errors.New("customhttp: invalid character in file path")
	}
	dir := string(d)
	if dir == "" {
		dir = "."
...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	...
	for {
		...
		//TODO 'Get' is supposed to be 'get' here, probably check again later
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time,
	...
	if size >= 0 {
		...
		println(" range req:", rangeReq)
		for i := 0; i < len(ranges); i++ {
			println("range start:", ranges[i].start, "range length:", ranges[i].length)
		}
		if err != nil {
			if err == errNoOverlap {
		...
		case len(ranges) == 1:
			...
			print("content range:", ra.contentRange(size))
